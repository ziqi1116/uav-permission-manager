-- =============================================================================
-- 无人机飞行管理系统 - 最终整合安装脚本（仅包含本项目新增/修改内容）
-- 前置条件：已执行若依基础 SQL（如 ry_20250522.sql），不在此脚本中重复建若依原始表
-- 本脚本包含：
--   1）业务表（无人机 / 申请 / 权限 / 监控 / 违规）
--   2）业务字典
--   3）角色（approver / flight_user / drone_user）
--   4）菜单结构修复（删除「无人机飞行管理」「统计报表」「若依官网」，业务菜单扁平为一级）
--   5）首页菜单与权限修复（admin + drone_user 均可访问）
--   6）drone_user 角色菜单精简：仅 首页 / 无人机管理 / 飞行申请 / 飞行权限 / 监控管理
--   7）模拟 3 条飞行任务，每条 20 条轨迹 + 违规数据
--   8）测试账号 user1（密码使用系统初始密码规则，默认 123456）
--   9）开启注册开关（sys.account.registerUser = true）
-- 可重复执行：表采用 DROP+CREATE；菜单/角色/权限/数据 通过 NOT EXISTS 或 INSERT IGNORE 保障幂等
-- =============================================================================
SET NAMES utf8mb4;

-- =============================================================================
-- 一、业务表（仅本项目新增表，不包含若依原始表）
-- =============================================================================
...
-- =============================================================================
-- 四、菜单结构修复（删除父级与无关菜单，业务菜单扁平化为一级）
-- =============================================================================
-- 删除「若依官网」
DELETE FROM sys_role_menu WHERE menu_id IN (
    SELECT menu_id FROM (SELECT menu_id FROM sys_menu WHERE path = 'http://ruoyi.vip' OR (menu_name = '若依官网' AND parent_id = 0)) t
);
...
DELETE FROM sys_role_menu WHERE menu_id IN (
    SELECT menu_id FROM (SELECT menu_id FROM sys_menu WHERE path = 'statistics' OR (parent_id = @stats_id AND @stats_id IS NOT NULL)) t
);
DELETE FROM sys_menu WHERE path = 'statistics' OR (parent_id = @stats_id AND @stats_id IS NOT NULL);

-- 创建业务父级菜单「无人机飞行管理」（若已存在则沿用），再统一扁平化
INSERT INTO sys_menu (menu_name, parent_id, order_num, path, component, query, route_name, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, remark)
SELECT '无人机飞行管理', 0, 5, 'drone', NULL, '', '', 1, 0, 'M', '0', '0', '', 'guide', 'admin', sysdate(), '无人机飞行权限与审批'
FROM (SELECT 1) t WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE path = 'drone' AND parent_id = 0 LIMIT 1);
SET @parent_id = (SELECT menu_id FROM sys_menu WHERE path = 'drone' AND parent_id = 0 LIMIT 1);

-- 业务菜单（无人机管理 / 飞行申请 / 审批管理 / 权限查看 / 监控管理 / 违规管理）
...
SET @drone_menu = (SELECT menu_id FROM sys_menu WHERE path = 'droneInfo' AND (parent_id = @parent_id OR parent_id = 0) LIMIT 1);
INSERT INTO sys_menu (menu_name, parent_id, order_num, path, component, query, route_name, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, remark) VALUES
...
ON DUPLICATE KEY UPDATE menu_name = VALUES(menu_name);
SET @app_menu = (SELECT menu_id FROM sys_menu WHERE path = 'application' AND (parent_id = @parent_id OR parent_id = 0) LIMIT 1);
INSERT INTO sys_menu (menu_name, parent_id, order_num, path, component, query, route_name, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, remark) VALUES
...
ON DUPLICATE KEY UPDATE menu_name = VALUES(menu_name);
SET @approve_menu = (SELECT menu_id FROM sys_menu WHERE path = 'approve' AND (parent_id = @parent_id OR parent_id = 0) LIMIT 1);
INSERT INTO sys_menu (menu_name, parent_id, order_num, path, component, query, route_name, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, remark) VALUES
...
ON DUPLICATE KEY UPDATE menu_name = VALUES(menu_name);
SET @perm_menu = (SELECT menu_id FROM sys_menu WHERE path = 'permission' AND (parent_id = @parent_id OR parent_id = 0) LIMIT 1);
INSERT INTO sys_menu (menu_name, parent_id, order_num, path, component, query, route_name, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, remark) VALUES
...
ON DUPLICATE KEY UPDATE menu_name = VALUES(menu_name);
SET @violation_menu = (SELECT menu_id FROM sys_menu WHERE path = 'violation' AND (parent_id = @parent_id OR parent_id = 0) LIMIT 1);
INSERT INTO sys_menu (menu_name, parent_id, order_num, path, component, query, route_name, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, remark) VALUES
...
-- 将业务菜单全部扁平化为一级菜单：parent_id = 0，并删除父级「无人机飞行管理」
...
-- =============================================================================
-- 五、首页菜单及权限修复（admin、drone_user 均可访问）
-- =============================================================================
INSERT INTO sys_menu (menu_name, parent_id, order_num, path, component, query, route_name, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, remark)
...
SET @mid_index = (SELECT menu_id FROM sys_menu WHERE path = 'index' AND parent_id = 0 LIMIT 1);
INSERT IGNORE INTO sys_role_menu (role_id, menu_id) SELECT 1, @mid_index FROM (SELECT 1) t WHERE @mid_index IS NOT NULL;

-- =============================================================================
-- 六、drone_user 角色菜单：仅 首页 / 无人机管理 / 飞行申请 / 飞行权限 / 监控管理
-- =============================================================================
...
-- =============================================================================
-- 七、模拟测试数据：3 条飞行任务，每条 20 条轨迹 + 部分违规
-- =============================================================================
...
SELECT 'M1-正常', 'Matrice 300', 'SN-FIN-T01', @admin_id, '0', '任务1', 'admin', NOW()
FROM (SELECT 1) t WHERE @admin_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM drone_info WHERE serial_number = 'SN-FIN-T01');
...
SELECT @a1, '东湖A区', 120, NOW(), DATE_ADD(NOW(), INTERVAL 2 HOUR), '0', 'admin', NOW()
FROM (SELECT 1) t WHERE @a1 IS NOT NULL AND NOT EXISTS (SELECT 1 FROM flight_permission WHERE application_id = @a1);
...
-- 任务 2：超高违规
INSERT INTO drone_info (drone_name, drone_model, serial_number, owner_id, status, remark, create_by, create_time)
SELECT 'M2-超高', 'Phantom 4', 'SN-FIN-T02', @admin_id, '0', '任务2', 'admin', NOW()
FROM (SELECT 1) t WHERE @admin_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM drone_info WHERE serial_number = 'SN-FIN-T02');
...
INSERT INTO flight_permission (application_id, approved_area, approved_height, approved_start_time, approved_end_time, permission_status, create_by, create_time)
SELECT @a2, '东湖B区', 100, NOW(), DATE_ADD(NOW(), INTERVAL 2 HOUR), '0', 'admin', NOW()
FROM (SELECT 1) t WHERE @a2 IS NOT NULL AND NOT EXISTS (SELECT 1 FROM flight_permission WHERE application_id = @a2);
...
SELECT @p2, 'over_height', '飞行高度超过批准值100m', 114.362, 30.52, '0', 'system', NOW()
FROM (SELECT 1) t WHERE @p2 IS NOT NULL AND NOT EXISTS (SELECT 1 FROM violation_record WHERE permission_id = @p2 AND violation_type = 'over_height');

-- 任务 3：越界违规
INSERT INTO drone_info (drone_name, drone_model, serial_number, owner_id, status, remark, create_by, create_time)
SELECT 'M3-越界', 'Mavic 3', 'SN-FIN-T03', @admin_id, '0', '任务3', 'admin', NOW()
FROM (SELECT 1) t WHERE @admin_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM drone_info WHERE serial_number = 'SN-FIN-T03');
...
INSERT INTO flight_permission (application_id, approved_area, approved_height, approved_start_time, approved_end_time, permission_status, create_by, create_time)
SELECT @a3, '东湖C区', 80, NOW(), DATE_ADD(NOW(), INTERVAL 2 HOUR), '0', 'admin', NOW()
FROM (SELECT 1) t WHERE @a3 IS NOT NULL AND NOT EXISTS (SELECT 1 FROM flight_permission WHERE application_id = @a3);
...
INSERT INTO violation_record (permission_id, violation_type, description, longitude, latitude, handle_status, create_by, create_time)
SELECT @p3, 'out_of_bounds', '偏离批准空域', 114.468, 30.50, '0', 'system', NOW()
FROM (SELECT 1) t WHERE @p3 IS NOT NULL AND NOT EXISTS (SELECT 1 FROM violation_record WHERE permission_id = @p3 AND violation_type = 'out_of_bounds');

-- =============================================================================
-- 八、测试用户 user1（默认密码遵循系统初始密码配置：sys.user.initPassword，默认为 123456）
-- =============================================================================
...
-- =============================================================================
-- 结束。执行顺序建议：
--   1）执行若依官方基础脚本：ry_20250522.sql
--   2）执行本脚本：drone_system_final_install.sql
-- =============================================================================
